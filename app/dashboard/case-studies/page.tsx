  ...
  return (
    <div>
      ...
      {editingId !== null && (
        <form onSubmit={handleSave} className="mt-6 space-y-4 rounded-xl border border-neutral-200 bg-white p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <label className={labelClass}>
              Reference
              <input className={inputClass} value={form.ref} onChange={(e) => setForm({ ...form, ref: e.target.value })} placeholder="CS-07" required />
            </label>
            <label className={labelClass}>
              Category
            ...
            <label className={labelClass}>
              Pages
              <input className={inputClass} value={form.pages} onChange={(e) => setForm({ ...form, pages: e.target.value })} placeholder="12 pp" />
            </label>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <UploadField label="Cover image" kind="image" value={form.image} onChange={(url) => setForm({ ...form, image: url })} />
            <UploadField label="PDF file" kind="document" value={form.pdfUrl} onChange={(url) => setForm({ ...form, pdfUrl: url })} />
          </div>
      ...
      {items === null ? (
        ...
        <p className="mt-8 text-sm text-neutral-500">No case studies yet.</p>
      ) : (
