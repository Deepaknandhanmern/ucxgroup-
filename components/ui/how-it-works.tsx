...
const Card = ({
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-neutral-900 p-3 rounded-[25px] shadow-[0px_10px_20px_0px_#D3D3D3] dark:shadow-none border border-neutral-100 dark:border-neutral-800">
        <Pin className={`w-8 h-8 ${textColor} z-20 mb-6 mx-auto`} />
        <div
          className={`${bgColor} border ${borderColor} rounded-[15px] p-6 h-full flex flex-col relative overflow-hidden`}
        >
          <span
            className={`${textColor} text-4xl font-bold mb-5`}
            style={{
              fontFamily: "'Getho','Atyp Display',sans-serif",
            }}
          >
            {number}
          </span>
          <h3
            className="text-xl font-bold uppercase tracking-tight text-neutral-800 dark:text-neutral-100 leading-tight mb-[10px]"
            style={{ fontFamily: "'Getho','Atyp Display',sans-serif" }}
          >
            {title}
          </h3>
...
const DEFAULT_CARD_POSITIONS: StepPosition[] = [
  { className: "md:absolute md:top-0 md:left-[15%]", rotate: "md:rotate-8" },
  {
    className: "md:absolute md:top-[120px] md:right-[15%]",
    rotate: "md:-rotate-8",
  },
  { className: "md:absolute md:top-[450px] md:left-[15%]", rotate: "md:rotate-8" },
  {
    className: "md:absolute md:top-[570px] md:right-[10%]",
    rotate: "md:-rotate-8",
  },
  { className: "md:absolute md:top-[850px] md:left-[15%]", rotate: "md:rotate-8" },
];
...
export default function HowItWorks({
  ...
  if (trackHeight) height = trackHeight;
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      >
        <div className="from-[#F3F1E6] dark:from-black pointer-events-none absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r"></div>
        <div className="from-[#F3F1E6] dark:from-black pointer-events-none absolute inset-y-0 right-0 w-1/2 bg-gradient-to-l"></div>
              ...
              >
                {(() => {
                  const pathD = data.reduce((acc, _, index) => {
                    ...
                    if (index === 0)
                      return "M 290 103 C 500 103, 550 186, 710 186"; // 1 -> 2
                    if (index === 1)
                      return acc + " C 850 186, 500 241, 290 310"; // 2 -> 3
                    if (index === 2)
                      return acc + " C 290 413, 550 496, 750 496"; // 3 -> 4
                    if (index === 3)
                      return acc + " C 950 496, 500 551, 290 585"; // 4 -> 5
                    return acc;
                  }, "");
                  return (
                    <m.path
